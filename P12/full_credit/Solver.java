...
public class Solver {
    public static void main(String[] args) {
        ...
        if (args.length == 0) {
            System.err.println(
                    "usage: java Solver [--precision p] [--threads t] [--slices s] [--min n] [--max m] [--log] file.poly...");
            System.err.println(
                    "  precision - solve until within this much of the actual root (default " + precision + ")");
            System.err
                    .println("  threads   - use this many threads to solve each polynomial  (default " + threads + ")");
            System.err
                    .println("  slices    - divide min-max range into this many subranges   (default " + slices + ")");
            ...
            System.err.println("  log       - print log messages to STDERR                    (default "
        ...
        while (++index < args.length) {
            if (args[index].equals("--precision"))
                precision = Double.parseDouble(args[++index]);
            else if (args[index].equals("--threads"))
                threads = Integer.parseInt(args[++index]);
            else if (args[index].equals("--slices"))
                slices = Double.parseDouble(args[++index]);
        ...
        if (fail)
            System.exit(-1);
        System.out.println("\nthreads = " + threads + ": range " + min + " to " + max + ", " + slices
                + " slices, precision = " + precision + "\n");
